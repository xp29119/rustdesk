...
echo "[7] Pre-check login & dialog"
rg -n "connect\(BuildContext|showLoginRequiredDialog\(|dialogButton\(translate\('go_to_login'\)|dialogButton\(translate\('Cancel'\)" flutter/lib/common.dart || true

echo "[8] Fallback server error dialog"
rg -n "Connection failed, please login!|login_required_dialog_title2|login_required_dialog_body2|_loginPromptShown" flutter/lib/desktop/pages/desktop_home_page.dart || true
